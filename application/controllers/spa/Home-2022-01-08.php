		...
		$this->data['salesdetails'] = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_sale_order_details_by_gso',$array_sale_detail);
			...
			//p(json_decode($this->data['statedata']));exit;
			 ...
			 if(!empty($this->session->userdata('s_category')) and $this->session->userdata('s_category')=='Lounge')
				  {
				  $this->load->view('checkout-search-lounge',$this->data);
				  }
				  else
				  {
	...
	function updatesession()
	{
		...
		if(!empty($product_name))
		{
			...
			if($page=='Search')
			{
				...
				{
					$array[] = array('pid'=>$a_pid, "pname"=>$a_pname,'ptype'=>'Adults','prate'=>$a_rate,'pqty'=>$s_adult,'pprice'=>$s_adult*$a_rate,'ptaxrate'=>$a_tax,'ptaxprice'=>$s_adult*$a_tax);
				}
...
function placeorderlounge()
	{
		...
		if($_POST)
		{
			...
			redirect(base_url('home/ordersummary/'.$o_booking_id.'/'.$booking_id));
		}
	}
	function placeorder()
	{
	...
	{
		$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3))); 
		$db_id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(4))); 
		if($_POST)
		{
			//p($_POST);
...
function get_booking_data()
	{
		...
		//p(json_decode($this->data['datalist']));
		 echo $this->load->view('ajax_booking_summary', $this->data, TRUE );
	}	
