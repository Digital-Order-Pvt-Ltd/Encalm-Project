...
function booking_confirm()
	...
	public function login() 
	{
		if($_POST)
		{
			...
			$country = json_decode(curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_country',array()));
			foreach ($country->datalist as $item) {
				if ($item->id == "8097") {
					echo $item->content;
				}
			}
			  ...
			  //p($datalist);exit;
				...
				if($loginfrom=='Login')
				 redirect(base_url('spa/home/index'));
					...
					redirect(base_url('spa/home/confirm'));
			}
			else{
				...
				if($loginfrom=='Login')
				 redirect(base_url('spa/user/login'));
				else
					redirect(base_url('spa/home/confirm'));
		  ...
		  if(!empty($json_decode->id) and (int) $json_decode->id  and $json_decode->id > 0)
				{
					...
					if(empty($this->input->post( 'create_order' )))
					{
						...
						redirect(base_url('spas/home/confirm'));
					}
				...
				if(!EMPTY($order_id) and  (int)$order_id)
				{
					for($i=0;$i<count($s_select_product_details);$i++)
					 ...
					 $array=array(
						...
						redirect(base_url('spas/home/checkout/'.$order_id.'/'.$insert_id));
						}
					}
