...
class User extends CI_Controller {
 public function __construct() {
	...
	public function myprofile() {
		...
		$cdetails = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_customer_details',array('id'=>$o_id));
		$serveruserdata = json_decode($cdetails);
		$this->data['serveruserdata'] = $serveruserdata->datalist[0];
		if($_POST)
		{
		...
		$this->db->update('tbl_users',$array, array('id'=>$customer_id));
			...
			redirect(base_url("atithya/user/myprofile"));
		}
		else
		{
		$this->load->view('atithya/profile',$this->data);
		}
	}
...
$this->data['datalist'] = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_sale_order_list_by_partner',$array);
		...
		$this->load->view('atithya/booking-list', $this->data );
	} 
	public function bookingdetail() {
		...
		$this->load->view('atithya/booking', $this->data );
		}
		else
		{
 ...
 function booking_cancel()
	{
		...
		$this->session->set_flashdata("success", "Booking Cancellation Request has been submitted successfully!");
		redirect(base_url('atithya/user/bookingdetail/'.$id));
	}
function booking_confirm()
	...
	public function myguest() 
	{
		...
		//p($this->data['datalist']);exit;
		 $this->load->view('atithya/guest', $this->data );
	...
	public function changepassword() {
		...
		if($_POST) {
			...
			{
				$this->load->view('atithya/change-password', $this->data);
			} else {
				$session_customer_data = !empty($this->session->userdata("s_customer_data"))?$this->session->userdata("s_customer_data"):array();
				...
				{
					redirect(base_url('atithya/user/changepassword'));
				} 
				else 
				{
					redirect(base_url('atithya/user/changepassword'));
	...
	public function checkout() {
		...
		$this->load->view('atithya/checkout-search');
	}
			  ...
			  //p($datalist);exit;
				...
				$school  =  $this->db->query($sql)->row(); 
                   ...
                   /* $data = array(
					...
					$state_id = $datalist->state_id;
					$title = $datalist->title;
					$data = array(
                        'customer_id' => !empty($datalist->id)?$datalist->id:0,
                        'o_id' => $datalist->id,
				...
				if($loginfrom=='Login')
				 redirect(base_url('atithya/home/index'));
					...
					redirect(base_url('atithya/home/confirm'));
			}
			else{
				...
				if($loginfrom=='Login')
				 redirect(base_url('atithya/user/login'));
				else
					redirect(base_url('atithya/home/confirm'));
...
function adduser()
{
	...
	if($_POST)
		...
		$userdatalist = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_customer_duplicacy',array('email_id'=>$this->input->post( 'emailid' )));
		...
		'tnc'=>!empty($this->input->post( 'terms_conditions' ))?$this->input->post( 'terms_conditions' ):'no',
		'mkt_comm'=>!empty($this->input->post( 'marketing_communication' ))?$this->input->post( 'marketing_communication' ):'no'
		);
		...
		if(!empty($this->input->post( 'create_order' )) and $this->input->post( 'create_order' )=='yes')
			{	
					 ...
					 $sql  =  "SELECT *, DATE_FORMAT(dob,'%d-%b-%Y') as date_of_birth, (YEAR(current_date()) - YEAR(dob)) as current_age FROM `tbl_users` WHERE o_id='".$json_decode->id."'";
						$school  =  $this->db->query($sql)->row();
					if(!empty($school))
					{
					$data = array(
				...
				if(!EMPTY($order_id) and  (int)$order_id)
				{
					for($i=0;$i<count($s_select_product_details);$i++)
					 ...
					 if(empty($session_customer_data))
					 	$session_customer_data = !empty($this->session->userdata("s_customer_data"))?$this->session->userdata("s_customer_data"):array();
					 $array=array(
							'status'=>'0',
		...
		{
			$this->session->set_flashdata("success", "This email id is already registered with us!");
			redirect(base_url('atithya/home/confirm'));
		}
	}
...
function removeguest()
{
 ...
 redirect(base_url("atithya/user/myguest"));
}
function deleteguest()
{
	...
	//  p( $data);
	  $session_customer_data = !empty($this->session->userdata("s_customer_data"))?$this->session->userdata("s_customer_data"):array();
		...
		echo $this->load->view('atithya/ajax_guest_list', $this->data, TRUE );
}
function get_your_guest_list()
{
	...
	function logout()
	{
	   ...
	   return redirect()->to(base_url('atithya/user/login'));
...
public function signup() 
	{
	  ...
	  if(!empty($message) and $message=='data not found.')
	  {
		...
		{
			$this->session->set_flashdata("success", "This email id is already registered with us!");
			redirect(base_url('atithya/user/signup'));
		}
		}
		else
		{
			$this->load->view('atithya/signup',$this->data);
		}
		//$this->load->view('atithya/checkout-search');
	}
	...
	public function forgot() 
	{
		...
		if($_POST)
		{
			...
			if(!empty($emailid))
			{
				...
				if(!empty($json_decode->status) and $json_decode->status==true)
				{
					...
					redirect(base_url('atithya/user/thankyou'));
				}
				else
				{
					$this->session->set_flashdata("success",$message);
					redirect(base_url('atithya/user/forgot'));
				}
			}
			else
			{
		...
		{
			$this->load->view('atithya/forgot');
		}
		
	}
	  
	public function thankyou() {
		$this->load->view('atithya/forgot_thankyou');
	}
	
	public function invoicen() {
		//check_login_user();
		$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(4))); 
		$param = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(5))); 
		...
		//p($array);
		$this->data['param']=!empty($param)?$param:1;
			$this->data['datalist'] = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_sale_order_details_for_PDF_pi',$array);
			if($_POST)
			{ 
				 $this->load->library('pdf');
				$html = $this->load->view('atithya/invoice_pdf', $this->data, true);
				$this->pdf->createPDF($html, $id, 1);
			}
			else
			{
				$this->load->view('atithya/proformainvoice', $this->data);
			}
	...
	public function invoice() {
		...
		//p($array);
		$this->data['param']=!empty($param)?$param:1;
			$this->data['datalist'] = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_sale_order_details_for_PDF',$array);
			if($_POST)
			{ 
				 $this->load->library('pdf');
				$html = $this->load->view('atithya/invoice_pdf', $this->data, true);
			...
			{
				$this->load->view('atithya/invoice', $this->data);
			}
	...
	public function spacnfvoucher() {
		...
		$this->data['param']=!empty($param)?$param:1;
			...
			{ 
					$this->load->library('pdf');
				$html = $this->load->view('atithya/invoice_pdf_spa', $this->data, true);
				$this->pdf->createPDF($html, $id, 1);
			}
			else
			{
				$this->load->view('atithya/invoicespa', $this->data);
			}
