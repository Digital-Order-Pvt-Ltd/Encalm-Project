 ...
 public function __construct() {
		  ...
		  $s_category = !empty($this->session->userdata("s_category"))?$this->session->userdata("s_category"):'';
			if(!empty($s_category))
			{
					...
					if(!empty($allsearchdata)){
						unset($_SESSION['reschedule_posted']);
						unset($_SESSION['rescheduling_charges_total']);
						unset($_SESSION['reschedule_booking_id']);
						
						unset($_SESSION['prd']);
						unset($_SESSION['amount_old']);
	...
	public function search() 
	{
			...
			$service_airport_id = 0;			
			 ...
			 {	 
				 ...
				 if($category=='Lounge')
				 {
					...
					$DepartureTime = !empty($this->input->post( 'ArrivalTime' ))?$this->input->post( 'ArrivalTime' ):'';
					$ArrivalTime='';
					$ServiceTime = !empty($this->input->post( 'ServiceTime' ))?date('H:i',strtotime($this->input->post( 'ServiceTime' ))):'';
					$d_arrival_time = $ServiceTime;

					$ToAirport = '';

					$FromAirport = !empty($this->input->post( 'LoungeServiceAirport' ))?$this->input->post( 'LoungeServiceAirport' ):'';
					$SectorName = !empty($this->input->post( 'SectorName' ))?$this->input->post( 'SectorName' ):'';
					$LoungeHours = !empty($this->input->post( 'LoungeHours' ))?$this->input->post( 'LoungeHours' ):'';

					$service_date_time = !empty($this->input->post( 'ServiceTime' ))?date('Y-m-d H:i',strtotime($this->input->post( 'ServiceTime' ))):'';
				  
					 $service_airport_id = $to_airport_id;			
					 $tm = !empty($LoungeHours)?$LoungeHours:'';
					 $duration = str_replace(' Hrs','',$tm);
					 $order_departure_term = !empty($this->input->post( 'd_departure_term' ))?$this->input->post( 'd_departure_term' ):'';
				 }
		...
		//exit;
				...
				$this->load->view('atithya/travel-search',$this->data);
		}
		 else
		 {
			 redirect(base_url('atithya/home/index'));
		 }
	...
	public function addons() 
	{
		...
		{
			if(strtolower($this->session->userdata("s_category")) == 'lounge') 
			$id = '-1'; 
	...
	//p($this->session->all_userdata());exit;  
			...
			if(strtolower($this->session->userdata("s_category")) == 'lounge') {
				$this->load->view('atithya/addon-search-lounge',$this->data);
			}
			else {
		...
		if($_POST)
		{
			...
			if(!empty($session_customer_data) and !empty($s_select_product_details))
			{
				...
				if(!EMPTY($order_id) and  (int)$order_id)
				{
				  ...
				  redirect(base_url('atithya/home/checkout/'.$order_id.'/'.$insert_id));
				}
				else
				{
	...
	function updatesession()
	{
		...
		if(!empty($product_name))
		{
			...
			if($page=='Search')
			{
				...
				$data =  curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_product_variant_details',array('id'=>$product_id));
				 ...
				 date_default_timezone_set('Asia/Kolkata');
					...
					if(!empty($json_decode))
					{
						...
						// Given service date and time
						$s_service_date_time = $this->session->userdata("s_service_date_time");
					
						$s_arriving_airport_at = $this->session->userdata("s_arriving_airport_at");
						if($s_arriving_airport_at ==='Indira Gandhi International Airport'){
							$pidd = 2336;
						}elseif($s_arriving_airport_at ==='Rajiv Gandhi  International Airport'){
							$pidd = 2337;
						}else{
							$pidd = 12956;
						}
						// Convert the given date and time to a Unix timestamp
						$service_timestamp = strtotime($s_service_date_time);
						$twelve_hours_before_service = strtotime("-12 hours", $service_timestamp);
						$current_timestamp = time();					

						$array_within_12_hours = array();
						// Check if the difference is less than or equal to 12 hours (which is 12 hours * 60 minutes * 60 seconds)
						if  ($twelve_hours_before_service < $current_timestamp)  {
								$a_pid = $pidd;
								 $a_pname = 'SURCHARGE (REQUESTS WITHIN 12 HRS)';
								 $a_rate = 500;
								 $a_rate_surcharge = 500;
								 $a_tax = 80;
								 $a_tax_surcharge = 500;
							$array_within_12_hours[] = array(
								'pid' => $a_pid,
								'pname' => $a_pname,
								// 'ptype' => 'Adult',
								'prate' => $a_rate_surcharge,
								'pqty' => 1,
								'pprice' => $a_rate_surcharge,
								'ptaxrate' => $a_tax_surcharge,
								'ptaxprice' => $a_tax_surcharge
							);
				...
				// $array_m = array_merge($array,$array_within_12_hours);
				// $array = $array_m;
				  $this->session->set_userdata('s_select_product_details',$array );
...
function placeorderlounge()
				 ...
				 if(!empty($this->input->post( 'adult_first_name' )))
				 {
					 for($i=0;$i<count($this->input->post( 'adult_first_name' ));$i++)
					 {
						...
						$o_g_array=array(
							...
							'age'=>$age,
							'mobile'=>$this->input->post( 'adult_passport')[$i],
	...
	function ordersummary()
	{
		...
		$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(4))); 
		$booking_id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(5))); 
		$obid_reschedule = $_SESSION['reschedule_posted'];
		if($obid_reschedule){
			$this->data['reschedule_data'] = $this->getOrderData($obid_reschedule);
		}
		...
		//////////////////////////////////////////////////////
		if(strtolower($this->session->userdata('s_category'))!='spa' && strtolower($this->session->userdata('s_category'))=='lounge'){
			$array = array(
			'sale_id'=>$id ); 
			$this->data['datalist'] = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_sale_order_details_by_method',$array);
		} else {
			$array = array(
			'id'=>$id ); 
...
//Avaan Booking Code Block
        // $allsearchdata =  json_decode($this->data['datalist']);
		// $custData = !empty($allsearchdata->custData)?$allsearchdata->custData:array();
		// $arr = ["sale_id" => $id, "booking_no" => $allsearchdata->mainData->datalist[0]->itinerary_no, "cust_name" => $custData->datalist[0]->name, 
		// "cust_mobile" => $custData->datalist[0]->mobile, "cust_email" => $custData->datalist[0]->email];
		// //print_r($arr);
		//  $avan_conf = curlPostRequest(
        //     COMPANY_API_KEY,
        //     COMPANY_AGENT_CODE,
        //     "initiate_avaan_booking",
        //     $arr
        // );
        // //print_r($avan_conf);exit();
		// if($avan_conf){
		//     $data=json_decode($avan_conf);
        //     $nid=md5($data->id); 
        //     $url='https://avaanexcess.com/bookings/encalm_bookings.php?encalm_booking_id='.$nid.'&cust_name='.$custData->datalist[0]->name.'&cust_phone='.$custData->datalist[0]->mobile.'&cust_email='.$custData->datalist[0]->email;
...
//Avaan Booking Code Block
        ...
        else
        {
            ...
            {
        		$this->load->view('atithya/order-summary-lounge', $this->data );
            }
            else
            {
...
function thankyou_old()
	{
		...
		if($_POST)
		{
			...
			$up_data = array(
					...
					"came_from"=>!empty($this->input->post( 'came_from' ))? $this->input->post( 'came_from' ):'',
			...
			$order_check =$this->db->query("select * from tbl_post_payment_details where sale_id='$o_booking_id' order by id desc limit 1");
			
			$order_check_id =$order_check->row();
		$count = $order_check->num_rows();
		...
		$paymentModel->insert_data($up_data);
		}
		else{
		    
			
			$this->db->update('tbl_post_payment_details',$up_data,array('id'=>$order_check_id->id));
		}
			
// 			if (!$order_check){
// 			$paymentModel->insert_data();
// 			}
// 			else{
// 				$order_check_id =$order_check->row();
...
// 			}
			...
			$this->session->set_flashdata("Failed", "Your Booking has been failed. <br/> Thank you!"); 
			if($payment_status=='Success')
			{
				...
				$api_json_post=$confirm_sale_order;
				$json_data_post = json_encode($_POST, true);
				$json_data_response = json_encode($up_data, true);
				
				$api_json_response=curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'confirm_sale_order',$confirm_sale_order);
				
				$this->db->query("update tbl_booking set status='1' where o_order_id=$id");
				
				$log_array = array(
				    "ip"=>$this->input->ip_address(),
				    "json_data_post"=>$json_data_post,
				    "json_data_response"=>$json_data_response,
				    "api_json_post"=>json_encode($api_json_post,true),
				    "api_json_response"=>$api_json_response,
				    "orderid"=>$id
				    );
				$this->db->insert('log_tracker',$log_array);
	...
	function confirmation()
	{
        ...
        //  $this->load->view('atithya/thank_you_pay', $this->data );   
        // }
        else
        {
		    $this->load->view('atithya/thankyou', $this->data );
...
// 		$amount_sess = 1;
		...
		$order_check =$this->db->query("select * from tbl_post_payment_details where sale_id='$sale_id'");
		...
		$paymentModel->insert_data($ins_data);
		}
		else{
			
			$this->db->update('tbl_post_payment_details',$ins_data,array('id'=>$order_check_id->id));
		}
		...
		if($posted_amount!=$amount_sess)
		{
			...
			echo "<script>alert('Do not be smart by tweaking any changes in request');window.location = 'https://www.encalm.com';</script>";
			exit();
		}
	...
	$merchant_data='';
	$working_key="552A85150EF5A7EC2904FB12F620131F";
	$access_code="AVCE40JJ39AZ52ECZA";
	...
	$form_data = "<form method='post' name='redirect' action='$url'> 
		<center>
		<h1> Please wait...</h1>
...
// 			{	
			...
			$order_check =$this->db->query("select id,merchant_ref, reference_no from tbl_post_payment_details where sale_id='$id' order by id desc limit 1");
		
				$old_ord = $order_check->row();
				$old_ord_id_seq = $old_ord->id;
			    $old_order_id = $old_ord->merchant_ref;
				$old_tracking_id = $old_ord->reference_no;
				
				// echo "".$old_tracking_id;
				// echo "select merchant_ref, reference_no from tbl_post_payment_details where sale_id='$id' order by id desc limit 1";
				// echo "<br >".$old_order_id;
				// exit();
				...
				'razorpay_payment_id'=>$bank_ref_no,
				...
				// $this->db->update('tbl_post_payment_details',$data,array('merchant_ref'=>$old_order_id, 'sale_id'=>$id));
				
				$this->db->update('tbl_post_payment_details',$data,array('id'=>$old_ord_id_seq));
					...
					if(!empty($id) and  !empty($db_id))
				{
				   ...
				   // $this->load->view('atithya/thankyou', $this->data );
				   redirect(base_url('atithya/home/failed/'.$id.'/'.$db_id));
				}
			if(!empty($this->session->userdata('s_category')) and strtolower($this->session->userdata('s_category'))=='spa')
			{
				$this->load->view('atithya/thankyou-spa', $this->data );
...
function failed()
{
    ...
    $this->data['datalist'] = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_sale_order_details_by_gso',$array);
    $this->session->set_flashdata("warning", "Your Booking has been cancelled/failed, Booking No: <br/> Thank you!"); 
    $this->load->view('atithya/thankyou', $this->data );
    			}
}
...
{           
			...
			$order_check =$this->db->query("select id,merchant_ref, reference_no from tbl_post_payment_details where sale_id='$id' order by id desc limit 1");
				...
				if($payment_status!=="Success" && $sess_sale_id==$order_id && $old_payid!=$tracking_id)
				{
				    $this->session->set_flashdata("failed", "Your Booking has been cancelled for Booking No:  !"); 
					if(!empty($id) and  !empty($db_id))
				{
					$this->db->update('tbl_booking',$array,array('id'=>$db_id,'o_order_id'=>$id));
					...
					$this->db->query("update tbl_booking set status='0' where o_order_id=$id");
...
function ccavenue_status_tracker()
{
	...
	$orderid = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(4)));
	$tracking_id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(5)));
// 	echo $orderid;
// 	echo $tracking_id;
// 	exit();
...
echo "Encrypted Request : ". $final_data;
$ch = curl_init();
...
function partial_cancel(){
    $orderid = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(4)));
    $array = array(
				'id'=>$orderid, 
			);
	print_r($array);		
    $data = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'save_sale_order_partial_cancellation',$array);
    print_r($data);
}

function avaan_booking() 
{
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Use the retrieved data to populate the $arr array
        $arr = array(
            "sale_id" => $sale_id,
    ...
    $link='https://avaanexcess.com/bookings/encalm_bookings.php?encalm_booking_id='.$id.'&cust_name='.$cust_name.'&cust_phone='.$cust_mobile.'&cust_email='.$cust_email;
    echo $link;
    }
...
public function getRescheduleCharges($productType) {
$service_time = !empty($this->input->post( 'service_time' ))?$this->input->post( 'service_time' ):0;
$amount = !empty($this->input->post( 'amount' ))?$this->input->post( 'amount' ):0;
$_SESSION['reschedule_booking_id'] = !empty($this->input->post( 'sale_id' ))?$this->input->post( 'sale_id' ):0;

// Call method to calculate charges based on $productType
// $charges = $this->calculateRescheduleCharges($productType,$bookingAmount, $hours);
$charges = $this->calculateRescheduleCharges($productType, $amount, $service_time);
// print_r($charges);
// // Load a view with the charges
$data['charges'] = $charges;
$this->load->view('atithya/reschedule_charges_view', $data);
// return $charges;
}

private function calculateRescheduleCharges($productType, $bookingAmount, $serviceTime) {
    // Get the current time
    $currentTime = time();
    $percentageSlots = [
        'silver' => [
            ['hours' => 72, 'percentage' => 3],
            ['hours' => 48, 'percentage' => 10],
            ['hours' => 24, 'percentage' => 30],
            ['hours' => 0, 'percentage' => 100],
        ],
        'elite' => [
            ['hours' => 24, 'percentage' => 0],
            ['hours' => 12, 'percentage' => 50],
            ['hours' => 0, 'percentage' => 100],
        ],
    ];
    $slots = $percentageSlots[$productType];
    $applicableSlot = null;

    foreach ($slots as $slot) {
        if ($currentTime <= strtotime("-{$slot['hours']} hours", $serviceTime)) {
            $applicableSlot = $slot;
			$applicableSlotS = $slot['hours'];
            break;
        }
    }
    // If no applicable slot is found, use the last slot
    if (!$applicableSlot) {
        $applicableSlot = end($slots);
    }
    $charges = ($applicableSlot['percentage'] / 100) * $bookingAmount;

	return ['service_slot' => $applicableSlotS, 'charges_percentage'=>$applicableSlot['percentage'],'charges'=>$charges];
}
    public function reschedulePost() {
       
        $toAirportId = $this->input->post('to_airport_id');
        $fromAirportId = $this->input->post('from_airport_id');
        $terminalId = $this->input->post('terminal_id');
        $terminalCode = $this->input->post('terminal_code');
        $terminalValue = $this->input->post('terminal_value');
        $airlineId = $this->input->post('airline_id');
        $airlineName = $this->input->post('airline_name');
        $arrivalTerm = $this->input->post('arrival_term');
        $departureTerm = $this->input->post('departure_term');
        $airportLocationCodeId = $this->input->post('airport_location_code_id');
        $airportLocationCode = $this->input->post('airport_location_code');
        $flightId = $this->input->post('flight_id');
        $arrivalTime = $this->input->post('arrival_time');
        $category = $this->input->post('category');
        $serviceDateTime = $this->input->post('service_date_time');
        $doa = $this->input->post('doa');
        $flightName = $this->input->post('FlightName');
        $arrivalTimeAlt = $this->input->post('ArrivalTime');
        $serviceTime = $this->input->post('ServiceTime');
		

		// Convert serviceDateTime to datetime format
        $serviceDateTime = DateTime::createFromFormat('d-m-Y H:i', $serviceDateTime);
        $serviceDateTimeFormatted = $serviceDateTime ? $serviceDateTime->format('Y-m-d H:i:s') : null;

        // Convert doa to date format
        $doa = DateTime::createFromFormat('d-m-Y', $doa);
        $doaFormatted = $doa ? $doa->format('Y-m-d') : null;
		$obid = $_SESSION['reschedule_booking_id'];
		$productType = $_SESSION['prd']? $_SESSION['prd']:null;
		$old_amount = $_SESSION['amount_old']?$_SESSION['amount_old']:0;
		$service_times = $_SESSION['serviceTime'];
		$charges = $this->calculateRescheduleCharges($productType, $old_amount, $service_times);
		$_SESSION['rescheduling_charges_total'] = $charges;
		$data = array(
			'sale_id' => $obid,
            ...
            'arrival_term' => $arrivalTerm,
            'departure_term' => $departureTerm,
            'airport_location_code_id' => $airportLocationCodeId,
            'airport_location_code' => $airportLocationCode,
            'flight_id' => $flightId,
            'arrival_time' => $arrivalTime,
            'category' => $category,
            'service_date_time' => $serviceDateTimeFormatted,
            'doa' => $doaFormatted,
            'flight_name' => $flightName,
            'arrival_time_alt' => $arrivalTimeAlt,
            'service_time' => $serviceTime,
			'old_amount' => $old_amount
        );

		
        $insert_data = $this->db->insert('rescheduling_price_check', $data);
        ...
        if ($insert_data) {
			...
			$array_reschedules[] = array(
				'pid' => $pidd,
				'pname' => "Rescheduling Charges",
				// 'ptype' => 'Adult',
				'prate' => $_SESSION['sess_charges'],
				'pqty' => 0,
				'price_subtotal' => $_SESSION['sess_charges'],
				'ptaxrate' => 0,
				'ptaxprice' => 0
			);
			// $array = !empty($this->session->userdata("s_select_product_details"))?$this->session->userdata("s_select_product_details"):array();
			// $array_m = array_merge($array,$array_reschedules);
			$array = $array_reschedules;
			$this->session->unset_userdata('s_select_product_details');
			$this->session->set_userdata('s_select_product_details',$array );
			
            // If insertion was successful, redirect
            $obid_db = $this->db->insert_id(); 
			$_SESSION['reschedule_posted'] = $obid_db;
            redirect(base_url('atithya/home/ordersummary/' . $obid . '/' . $obid_db));
        } else {
            // If insertion failed, handle the error (you can customize this part)
            $response = array('status' => 'error', 'message' => 'Failed to insert data into the database');
            echo json_encode($response);
        }
		
    }
	private function getOrderData($obid) {
	// Add your database query to fetch order data based on $obid
	// Example query:
	$query = $this->db->get_where('rescheduling_price_check', array('id' => $obid));
	
	// Return the fetched data
	return $query->row(); // Assuming you expect only one row of data
}
	public function reschedule($obid) 
	{
		//p($this->session->all_userdata()); 
		unset($_SESSION['validity']);
		...
		$this->load->view('atithya/reschedule_index');
	}
	...
	public function proformainvoice() 
	{
		if(isset($_POST['createpdf'])) {
			...
			$html = $this->load->view('atithya/proformainvoice', '', TRUE);
			$this->pdf->createPDF($html, '', 1);
		}
		else {
			$this->load->view('atithya/proformainvoice');
		}
	}
