		  ...
		  $s_category = !empty($this->session->userdata("s_category"))?$this->session->userdata("s_category"):'';
			if(!empty($s_category))
			{
					$unset_session = array(
						...
						//p($unset_session_11);
					$this->session->unset_userdata($unset_session_11);
			}
	...
	public function search() 
	{
		// p($_POST);exit;
		 if($_POST)
		 {
			 ...
			 $airline_id = !empty($this->input->post( 'airline_id' ))?$this->input->post( 'airline_id' ):1;
	...
	public function confirm() 
	{
		...
		if($_POST)
		{
			...
			if(!empty($session_customer_data) and !empty($s_select_product_details))
			{
				...
				'airport_location_id'=>!empty($this->session->userdata("s_airport_location_id"))?$this->session->userdata("s_airport_location_id"):29,
				...
				'arrival_time'=>'2021-11-18 15:30',
				'departure_time'=>'2021-11-18 11:00',
				'booking_made_from'=>'b2c',
				'from_airport_id'=>!empty($this->session->userdata("s_to_airport_id"))?$this->session->userdata("s_to_airport_id"):0,
				'to_airport_id'=>!empty($this->session->userdata("s_from_airport_id"))?$this->session->userdata("s_from_airport_id"):0,
