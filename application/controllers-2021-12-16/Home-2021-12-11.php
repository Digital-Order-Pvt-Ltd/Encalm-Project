...
'service_time'=>date('Y-m-d',strtotime($doa)).' '.$ServiceTime,
...
$this->data['flightdata'] = curlPostRequest(COMPANY_API_KEY,COMPANY_AGENT_CODE,'get_product_list_meet_greet',$array);
	...
	public function confirm() 
	{
		...
		if($_POST)
		{
			...
			if(!empty($session_customer_data) and !empty($s_select_product_details))
			{
				...
				'commitment_date'=>!empty($this->session->userdata("s_doa"))?date('Y-m-d',strtotime($this->session->userdata("s_doa"))).' '.$this->session->userdata("s_arrival_service_time"):'',
		...
		if($_POST)
		{
			...
			if($booking_id)
			{
				...
				if(!empty($this->input->post( 'placard_first_name' )) and !empty($o_booking_id))
				{
					...
					'country_code'=>!empty($this->input->post( 'placard_country' ))?$this->input->post( 'placard_country' ):'',
					//'email'=>'',
	...
	//		$this->load->view('thankyou',$this->data);
			redirect(base_url('home/thankyou/'.$o_booking_id));
		}
	}
	function thankyou()
	{
