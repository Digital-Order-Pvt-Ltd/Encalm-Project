                ...
                <div class="card-body ps-0">
                  ...
                  <div class="row mt-2">
                    <div class="col-md-12">
					  ...
					  <a onclick='redirecturl("<?=base_url("home/addons/".$val->id)?>",<?=$val->id?>,"<?=$val->name?>","<?=!empty($val->list_price)?$val->list_price:0?>","<?=!empty($val->service_delivery_time_hrs)?$val->service_delivery_time_hrs:0?>","<?=!empty($val->tax_price)?$val->tax_price:0?>")' class="btn btn-primary float-end" id="selected_id_<?=$val->id?>">Select</a> <a onclick="showDesciption(<?=$val->id?>)" class="small details" style="cursor:pointer">Details</a>
		...
		<div class="col-md-4 mb-4 ordersummary">
          <div class="card mb-3">
			...
			<div class="form-group col-md-12 pb-3">
			  <input type="text" id="lounge_service_time"  class="form-control datepicker1"  placeholder="Add Check In Date and Time*" />
			</div>
			
			<div class="form-group col-md-12">
				<span class="caret"></span>
				<select id="lengthofstay" class="form-control">
...
function redirecturl(url,product_id,product_name,product_price,service_duration,tax_price)
{
 ...
 $('#selected_id_'+product_id).click(false);
	...
	$.ajax({
			...
			dataType: "json",
			 	success: function(data) { 
				$('#selected_id_'+product_id).text('Select');	 
				 const item=data; 
				//alert(item.url);
