...
<div class="clearfix"></div>
        ...
        <div class="col-md-5 ordersummary">
			...
			<tbody>
			  ...
			  <tr>
				...
				foreach($itemData->datalist as $item){
					...
					?> <a  class=" RemoveCouponClass" onclick="RemoveApplyCoupon(<?php echo !empty($item->id)?$item->id:0?>,<?php echo !empty($mainData->datalist[0]->id)?$mainData->datalist[0]->id:0?>);">Remove</a></td>
		  ...
		  <a onclick="PayNow()" class="btn btn-primary w-100"  role="button" aria-disabled="false">Pay Now</a>
          </div>
        </div>
      </div>
    </div>
...
function PayNow(){ $(".razorpay-payment-button").trigger("click"); }
</script>
<?php $order_Id = 'ORD_'.time().'_'.rand(1000,9999).'_END'?>
<?php $keyId = 'rzp_test_WEQMD6ZP6Ih63X';?>
<?php $amount = !empty($NetAmount)?$NetAmount:0;?>
<form action="<?=base_url("home/thankyou/".$o_booking_id.'/'.$booking_id);?>" method="POST" id="autoclick">
<script
