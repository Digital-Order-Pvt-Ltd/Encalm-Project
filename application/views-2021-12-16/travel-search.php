                ...
                <div class="card-body ps-0">
                  ...
                  <div class="row mt-2">
                    <div class="col-md-12">
					  ...
					  <a onclick='redirecturl("<?=base_url("home/addons/".$val->id)?>",<?=$val->id?>,"<?=$val->name?>","<?=!empty($val->list_price)?$val->list_price:0?>","<?=!empty($val->service_delivery_time_hrs)?$val->service_delivery_time_hrs:0?>")' class="btn btn-primary float-end" id="selected_id_<?=$val->id?>">Select</a> <a onclick="showDesciption(<?=$val->id?>)" class="small" style="cursor:pointer">Details</a>
                        <div class="hover-info">
                          <div class="card">
                            <div class="card-header" > Included </div>
							<div id="more_desc_<?=$val->id?>"><?=$val->description?></div>
...
function redirecturl(url,product_id,product_name,product_price,service_duration)
{
 ...
 $('#selected_id_'+product_id).click(false);
	...
	$.ajax({
			...
			data: {'page':'Search','redirecturl':url,'product_name':product_name,"product_id":product_id,"product_price":product_price,"service_duration":service_duration},
			dataType: "html",
