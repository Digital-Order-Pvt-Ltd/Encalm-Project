<?php 
    // Get the current URI
      ...
      if (strpos($currentUri, 'spa') !== false) {
          ...
          $spa =true;
          $url = base_url('spa/user/login');
      } else {
          // The URI does not contain "spa"
          $spa =false;
          $url = base_url('atithya/user/login');
      }
  ?>
<form  id="userloginform"  name="" method="post" action="<?=$url?>" class="custom-search-from formfloats" enctype="application/x-www-form-urlencoded">
  <div class="row">
    <div class="col-md-6 border-end py-3">
      <div class="row">
        <div class="form-group col-md-12">
          <input type="text" class="form-control" id="username_login" name="username" placeholder="">
          <label class="floating-label">Email Id</label>
          <span style="font-weight: 700;font-size: small;color: red;display: none;" id="username_alert">Email Id is required</span>
        </div>
        <div class="form-group col-md-12">
          <input type="password" class="form-control" id="pwd_login" name="password" placeholder="">
          <label class="floating-label">Password</label>
          <span style="font-weight: 700;font-size: small;color: red;display: none;" id="login_password_alert">Password is required</span>
        </div>
        ...
        <div class="form-group col-lg-12 text-center">
          ...
          <a href="<?=base_url("atithya/user/forgot");?>" class="small d-inline ms-3" target="new">Forgot Password</a> </div>
      </div>
    </div>
...
{
	var error = false;
	$("#username_login").removeClass("focus");
	$("#pwd_login").removeClass("focus");
	 
	
	if($("#username_login").val() == "")
		{
			$("#username_login").addClass("focus");
      ...
      username_alert.style.display = "block";
		...
		if($("#pwd_login").val() == "")
		{
			$("#pwd_login").addClass("focus");
			$("#pwd_login").focus();
      const login_password_alert = document.getElementById("login_password_alert");
      login_password_alert.style.display = "block";
			error = true; 
		}
