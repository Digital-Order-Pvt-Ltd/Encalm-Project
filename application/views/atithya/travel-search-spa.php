  ...
  <section class="mamber-card mamber-card-checkout">
    <div class="container">
	  ...
	  <div class="col-md-8 mb-4">
				...
				if(!empty($allsearchdata->message) and $allsearchdata->message=='data found.'){
					...
					if(!empty($allsearchdata->datalist)){
						foreach($allsearchdata->datalist as $val){
							if(($val->available == 1) || (strtolower($this->session->userdata('s_category'))=='lounge'))
							{
								$min = 1;
          ...
          <div class="card">
            <div class="row">
              <!-- <div class="col-md-4">
				...
				<img class="card-img-top" src="data:image/png;base64,<?=$val->image_1920?>" alt="Card image cap"> 
			</div>-->
              <div class="col-md-12">
                <div class="card-body">
                  <div class="row">
                  ...
                  <div class="row mt-2">
                    <div class="col-md-12">
                      <div class="small">   </div>
					  <p><?=!empty($val->categ_id[1])?$val->categ_id[1]:''?></p>
					  <div class="hght"><?=$val->description?> </div>
					  ...
					  <!-- <a onclick='redirecturl("<?=base_url("atithya/home/addons/".$val->id)?>",<?=$val->id?>,"<?=$val->name?>","<?=!empty($val->list_price)?$val->list_price:0?>","<?=!empty($val->service_delivery_time_hrs)?$val->service_delivery_time_hrs:0?>","<?=!empty($val->tax_price)?$val->tax_price:0?>","<?=!empty($val->surcharge)?$val->surcharge:0?>")' class="btn btn-primary float-end serachselectclass" id="selected_id_<?=$val->id?>">Select</a> <a onclick="showDesciption(<?=$val->id?>)" class="small details" style="cursor:pointer">Details</a>
					 -->
					 <?php
					  if ($spa){
						$urls = base_url("spa/home/addons/".$val->id);
					  }
					  else{
						$urls = base_url("atithya/home/addons/".$val->id);
					  }
					  ?>
					  <a onclick='redirecturl("<?=$urls?>",<?=$val->id?>,"<?=$val->name?>","<?=!empty($val->list_price)?$val->list_price:0?>","<?=!empty($val->service_delivery_time_hrs)?$val->service_delivery_time_hrs:0?>","<?=!empty($val->tax_price)?$val->tax_price:0?>","<?=!empty($val->surcharge)?$val->surcharge:0?>")' class="btn btn-primary float-end serachselectclass" id="selected_id_<?=$val->id?>">Select</a> <a onclick="showDesciption(<?=$val->id?>)" class="small details" style="cursor:pointer">Details</a>
		...
		<?php }}}}}
		if($min==0) {
		        echo '<h4 class="text-danger">The slots are booked to capacity for the selected service time. Kindly connect with us on +91 9289366950 for the service availability check</h4>';
		} ?>
		</div>
        </div>
		...
		if(strtolower($this->session->userdata('s_category'))=='spa') { ?>
		<?php
		echo $this->session->userdata('s_category');
		?>
		  <?php if(!empty($this->session->userdata("s_customer_data"))){ ?>

		  <form method="post" action="<?=base_url('spa/home/confirm')?>" onsubmit="PleaseWaitShow();" id="addonform">
          ...
          <a href="<?=base_url('spa/home/confirm')?>"  class="btn btn-primary w-100" onclick="PleaseWaitShow();" id="addoncontinue">Continue >></a> 
		  <?php } ?>
...
function redirecturl(url,product_id,product_name,product_price,service_duration,tax_price,surcharge)
{
 ...
 $('#selected_id_'+product_id).click(false);
	...
	$.ajax({
			type: 'POST',
			url: "<?=base_url('spa/home/updatesession')?>",
			data: {'page':'Search','redirecturl':url,'product_name':product_name,"product_id":product_id,"product_price":product_price,"service_duration":service_duration,"tax_price":tax_price,"surcharge":surcharge},
			dataType: "json",
			 	success: function(data) { 
