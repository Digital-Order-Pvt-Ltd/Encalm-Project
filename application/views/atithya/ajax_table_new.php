				...
				//p($select_product_details);
				 ...
				 $NetAmountTax = array_sum(array_column($select_product_details,'ptaxprice'));
				  ...
				  if(!empty($select_product_details)){
					  ...
					  { ?>
                  <tr>
                    <td><?php echo $select_product_details[$i]['pname']; 
					if ($select_product_details[$i]['pqty'] && !in_array($select_product_details[$i]['pid'], [2336, 2337, 12956])) {
							echo '(' . $select_product_details[$i]['pqty'] . '&nbsp;' . $select_product_details[$i]['ptype'] . ')';
						} else {
							//echo $select_product_details[$i]['pid'];
						}
					  ?>
