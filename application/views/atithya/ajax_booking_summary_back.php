...
if(!empty($datalist))
{
	...
	//p($itemData);
	$booking_amt = round($mainData->datalist[0]->amount_total,0);
	$_SESSION['final_amount_resheduling'] = $booking_amt;
...
if(!empty($mainData)){
			...
			<tbody>
			  ...
			  <tr>
				...
				<?php  } else { ?>  
                  <tr>
					...
					<?php } }}?> 
					<?php 
					$reschedule_product = !empty($this->session->userdata("s_select_product_details"))?$this->session->userdata("s_select_product_details"):array();
					if(!empty($reschedule_product)){
						//print_r($reschedule_product);
						$NetAmount = $NetAmount + array_sum(array_column($reschedule_product,'price_subtotal'));
						$NetAmounts = $NetAmount-$booking_amt;
						$NetAmount = $NetAmounts;//number_format(max($NetAmount, $NetAmounts), 2);
						?>

					<tr>
						<td><?php 
						
							echo !empty($reschedule_product[0]['pname'])?$reschedule_product[0]['pname']:'NA'; 
							// echo "&nbsp;".$reschedule_product[0]['qty'].' Qty';
						?></td>
						<td align="right">&#8377; <?=$reschedule_product[0]['price_subtotal']?></td>
					</tr>
					<?php 
					}
					?>
			  ...
			  <tr>
					...
					<td align="right" class="bg-light">&#8377;<span id="netAmount"><?php echo $NetAmount+$booking_amt; ?></span>
			 ...
			 <tr>
			  ...
			  <tr>
					<td class="bg-light">Already Paid</td> 
					<td align="right" class="bg-light">&#8377;<?=$paid_amt = round(($sgst+$cgst+$NetAmount),0)+$_SESSION['final_amount_resheduling']-$booking_amt;?> </td>
			 </tr>
			 
			  <tr>
				<td class="bg-light">Final Amount</td>
				<td align="right" class="bg-light">&#8377;<span id="finalTotal">
					<?php 
					$_SESSION['final_amount_resheduling'] =  round($mainData->datalist[0]->amount_total,0);
				echo round(($sgst+$cgst+$NetAmount),0);?></span>
