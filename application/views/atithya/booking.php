...
<div class="clearfix"></div>
		...
		<?php } else {?>
      <!-- <p class="my-4 text-center"><a href="javascript:void(0)" class="reschargesBtn btn btn-primary" id="reschargesBtn" onclick="openCharges()" data-toggle="modal">Reschedule Service</a></p> -->
      <?php
$commitmentDate = !empty($mainData->datalist[0]->commitment_date) ? strtotime($mainData->datalist[0]->commitment_date) : 0;
$currentDate = strtotime('now');
//Enable below code on live:
//if ($commitmentDate > $currentDate) {
//Disable below and enable above to make it live
// if ($commitmentDate < $currentDate) {
    // The commitment date is greater than the current date and time, so display the button.
    echo '<p class="my-4 text-center"><a href="javascript:void(0)" class="reschargesBtn btn btn-primary" id="reschargesBtn" onclick="openCharges()" data-toggle="modal">Reschedule Service</a></p>';
    include 'reschedule_charges.php';
//}
?>
