...
<main id="hero" class="bg-white">
  <div class="bg-light pt-3 pb-0">
  	<div class="container">
  		<div class="section-title pb-3">
            <p>My Guest List &nbsp; &nbsp;<a style="float:right;" id="addnewguest" class="btn btn-primary"><i class="fas fa-plus"></i> Add New Guest</a></p>
  ...
  <section class="mamber-card">
    <div class="container search-box">
      <div class="row">
         <div class="col-lg-12">
        <p><strong>We request you keep your guests list upated with all correct details for helping us to serve you and your guests better.</strong></p>
        <table class="table table-striped table-bordered yTable pt-3 pb-3" cellpadding="0" cellspacing="0" width="100%">
  ...
  <tbody>
    ...
    <tr>
	  ...
	  <td><?=!empty($guest->dob)?date('d-M-Y',strtotime($guest->dob)):''?></td>
	  <td><?=!empty($guest->age)?$guest->age:''?></td>
       <td><?=!empty($guest->gender)?ucfirst($guest->gender):''?></td>
       <td><?=!empty($guest->passport)?$guest->passport:''?></td>
       <td>
	   <a onclick="EditGuest(<?=$guest->id?>)" class="btn btn-primary  btn-sm small"><i class="fas fa-edit"></i> Edit</a>
	   
	   <!--a href="<?=base_url('atithya/user/removeguest/'.$guest->id);?>" class="btn btn-danger  btn-sm small">Removed</a-->
  ...
  <div class="modal-dialog modal-dialog-centered  modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-body search-box px-4">
      	<a class="btn-close small closeBTN"> X </a>
        <h3 class="modal-title" id="exampleModalLabel">Add New Guest</h3>
		<div id="setGuestDataList">
        <form id="g_addguestform" name="" method="post" action="<?=base_url('atithya/user/addguest')?>" class="custom-search-from pt-3" enctype="application/x-www-form-urlencoded">
          <div class="row">
       ...
       <?php }}}?>
            ...
            <div class="form-group col-lg-1 text-center">
              <button type="button"  name="submit" class="btn btn-primary btn-sm small  float-end px-3" id="add_guests" onclick="save_guest_details('<?=base_url('atithya/user/addguest');?>','<?=base_url('atithya/user/myguest');?>')"> + ADD</button>
            </div>
</form>
    ...
    <div class="modal-content">
      <div class="modal-body search-box px-4">
		...
		<div id="setGuestDataList">
          <div class="row">
            ...
            <div class="form-group col-md-2 pd-10">
              <label for="" class="d-none">Title*</label>
              <span class="caret"></span>
              <select id="e_guest_title_1" name="" class="form-control d-inline-block  float-end">
                <option value="">Title*</option>
                 <?php if(!empty($salutation)){ $salutation_data =json_decode($salutation); 
			   if(!empty($salutation_data->datalist)){
				foreach($salutation_data->datalist as $val){
       ...
       <?php }}}?>
            ...
            <div class="form-group col-md-1 pd-10 text-center">
<a class="btn btn-primary btn-sm small  px-3 EditGuest_1" id="EditGuest_1"><i class="fas fa-user-edit"></i> Update</a>&nbsp;</div>

		<div class="clearfix mb-3"></div>
		 <div class="col-md-12" Style="color:red;text-align:center" id="setEditMsg"> </div>
		<div class="clearfix mb-3"></div>
	        <div class="form-group col-lg-12 text-center mt-3">
              <a class="btn btn-primary small closeBTN float-end"><i class="fas fa-times"></i> Close</a>
            </div>
          </div>
       </div>
      </div>
