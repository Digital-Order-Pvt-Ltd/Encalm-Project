                ...
                <div class="card-body ps-0">
                  ...
                  <div class="row mt-2">
                    <div class="col-md-12">
					  ...
					  <a onclick='redirecturl("<?=base_url("home/addons/".$val->id)?>",<?=$val->id?>,"<?=$val->name?>","<?=!empty($val->list_price)?$val->list_price:0?>","<?=!empty($val->service_delivery_time_hrs)?$val->service_delivery_time_hrs:0?>","<?=!empty($val->tax_price)?$val->tax_price:0?>","<?=!empty($val->surcharge)?$val->surcharge:0?>")' class="btn btn-primary float-end serachselectclass" id="selected_id_<?=$val->id?>">Select</a> <a onclick="showDesciption(<?=$val->id?>)" class="small details" style="cursor:pointer">Details</a>
